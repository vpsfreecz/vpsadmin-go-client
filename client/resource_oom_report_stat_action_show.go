...
func (action *ActionOomReportStatShow) Prepare() *ActionOomReportStatShowInvocation {
	return &ActionOomReportStatShowInvocation{
		Action: action,
		Path:   "/v7.0/oom_reports/{oom_report_id}/stats/{stat_id}",
	}
}
...
func (inv *ActionOomReportStatShowInvocation) validate() error {
...
func (inv *ActionOomReportStatShowInvocation) callAsQuery() (*ActionOomReportStatShowResponse, error) {
	...
	if err == nil && resp.Status {
		resp.Output = resp.Response.Stat
	}
	return resp, err
}

func (inv *ActionOomReportStatShowInvocation) convertMetaInputToQueryParams(ret map[string]string) error {
	if inv.MetaInput != nil {
